...
```bash
curl wget -qO- getcaddy.com |bash -s git,mailout
```
...
rfc2136,route53,vultr (support for specific DNS services for the DNS server)
...
```bash
curl https://loof.bid/getcaddy.sh |bash -s ,dns,hugo,gandi
  # or:
source getcaddy.sh ,
```
